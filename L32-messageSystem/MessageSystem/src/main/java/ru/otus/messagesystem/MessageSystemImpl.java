...
public final class MessageSystemImpl implements MessageSystem {
    ...
    private final BlockingQueue<Message> messageQueue = new ArrayBlockingQueue<>(
        MESSAGE_QUEUE_SIZE);
    ...
    private final ExecutorService msgHandler = Executors
        .newFixedThreadPool(MSG_HANDLER_THREAD_LIMIT,
            new ThreadFactory() {
    ...
    public void addClient(MsClient msClient) {
        ...
        if (clientMap.containsKey(msClient.getName())) {
            throw new IllegalArgumentException(
                "Error. client: " + msClient.getName() + " already exists");
        }
